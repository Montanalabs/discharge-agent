...
# Discharge-instruction agent

Untrusted input is a chart; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a chart
- **Closed type:** `type Decision = Send(Template) | Hold` where `Template` is closed
- **Sink / capability:** `grant sendInstruction` → `privileged { sendInstruction(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/discharge-agent/demo.sh
```
...
- `discharge-agent_safe.eide` — the correct design.
- `discharge-agent_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
